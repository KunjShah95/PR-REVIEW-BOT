...
export class AccessibilityAnalyzer extends BaseAnalyzer {
  ...
  initializeAccessibilityRules() {
    return {
      ...
      forms: [
        {
          pattern: /<input(?![^>]*(?:aria-label|aria-labelledby|id\s*=\s*['"][^'"]+['"])[^>]*<label)[^>]*>/gi,
          severity: 'high',
      ...
      if (/aria-[a-z]+\s*=\s*['"]/i.test(line)) {
        ...
        if (ariaMatch) {
          const validAria = [
            ...
            'aria-pressed', 'aria-live', 'aria-atomic', 'aria-busy', 'aria-controls',
            'aria-current', 'aria-haspopup', 'aria-invalid', 'aria-modal', 'aria-owns',
            'aria-placeholder', 'aria-required', 'aria-valuemax', 'aria-valuemin',
            'aria-valuenow', 'aria-valuetext', 'aria-level', 'aria-multiline',
            'aria-multiselectable', 'aria-orientation', 'aria-readonly', 'aria-relevant',
            'aria-roledescription', 'aria-rowcount', 'aria-rowindex', 'aria-rowspan',
            'aria-setsize', 'aria-sort', 'aria-colcount', 'aria-colindex', 'aria-colspan'
          ];
